...
def main():
	...
	for idx, rule in enumerate(module.params["rules"]):
		if (
			rule["preset"] is None
			and rule["match_user"] is None
			and rule["prefix"] is None
			and rule["catchall"] is None
		):
			module.fail_json(
				msg=f"rule[{idx}]: preset is None but any of the following are missing: match_user, prefix, catchall found in rules"
			)
	...
	try:
		...
		rules = [
			RuleSpec(**r).into_create_routing_request() for r in module.params["rules"]
		]

		extra_rules = [
			er.id for er in existing_rules if not any(r.matches(er) for r in rules)
		]
		missing_rules = [
			r for r in rules if not any(r.matches(er) for er in existing_rules)
		]
...
class RuleSpec:
	...
	preset: Optional[
		Literal["any_address", "catchall_except_valid", "prefix_match", "exact_match"]
	] = None
