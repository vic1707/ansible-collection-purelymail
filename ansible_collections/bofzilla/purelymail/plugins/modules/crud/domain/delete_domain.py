...
from ansible_collections.bofzilla.purelymail.plugins.module_utils.clients.types.requests import DeleteDomainRequest, ListDomainsRequest

DOCUMENTATION = r"""
---
module: delete_domain
short_description: Delete a domain from Purelymail
description:
  - Deletes a domain and all related configuration and users.
options:
  api_token:
...
def main():
	...
	try:
		...
		exists = any(d.name == name for d in existing_domains.domains)
		result = {"changed": exists}

		if module._diff:
			result["diff"] = {
				"before": existing_domains.as_api_response(),
				"after": existing_domains.filter(lambda d: d.name != name).as_api_response() if exists else existing_domains.as_api_response(),
			}
