...
from ansible_collections.bofzilla.purelymail.plugins.modules.routing_rules import (
	module_spec,
)
...
def test_any_address_preset():
	...
	assert 0 == len(res.error_messages)
...
def test_prefix_match_preset_requires_match_user():
	...
	assert "preset is prefix_match but any of the following are missing: match_user found in rules" == res.error_messages[0]
	assert 0 == len(res._warnings)
...
def test_exact_match_requires_match_user():
	...
	assert 1 == len(res.error_messages)
	assert "preset is exact_match but any of the following are missing: match_user found in rules" == res.error_messages[0]
	assert 0 == len(res._warnings)
